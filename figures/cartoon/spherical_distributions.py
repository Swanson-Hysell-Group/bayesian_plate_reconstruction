...
ax = fig.add_subplot(1,3,1, projection = ccrs.Orthographic(30,30))
c = ax.pcolormesh(mesh_lon,mesh_lat, uniform_vals,  transform=ccrs.PlateCarree(), cmap=cmap_green, vmin=-0.2, vmax=1.0, rasterized=True)
ax.scatter(u_samples[:,0], u_samples[:,1], transform=ccrs.PlateCarree(), c='darkgreen', edgecolors='darkgreen')
ax.gridlines()
ax.set_global()
ax.set_title("(a) Uniform")

ax = fig.add_subplot(1,3,2, projection = ccrs.Orthographic(30,30))
c = ax.pcolormesh(mesh_lon,mesh_lat, fisher_vals,  transform=ccrs.PlateCarree(), cmap=cmap_red, rasterized=True)
ax.scatter(vmf_samples[:,0], vmf_samples[:,1], transform=ccrs.PlateCarree(), c='darkred', edgecolors='darkred')
ax.gridlines()
ax.set_global()
ax.set_title("(b) Fisher")

ax = fig.add_subplot(1,3,3, projection = ccrs.Orthographic(30,30))
c = ax.pcolormesh(mesh_lon,mesh_lat, watson_vals,  transform=ccrs.PlateCarree(), cmap=cmap_blue, rasterized=True)
